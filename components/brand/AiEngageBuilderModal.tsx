...
import {
  ...
  ENGAGE_MIX_PRESETS,
  ENGAGE_PACE_OPTIONS,
  MAX_ENGAGE_QUESTIONS,
  MIN_ENGAGE_QUESTIONS,
  buildEngageAiPreview,
  defaultEngageAiConfig,
  engageMixForPreset,
  engageMixTotal,
  validateEngageAiConfig,
  type EngageAiConfig,
  type EngageMixPreset,
  type EngageTypeMix,
  type ReplaceMode,
...
interface Props {
  open: boolean
  onClose: () => void
  onSubmit: (config: EngageAiConfig) => void | Promise<void>
  loading?: boolean
...
type EngageQuestionTypeKey = keyof EngageTypeMix

const TYPE_LABELS: Record<EngageQuestionTypeKey, string> = {
  mcq: 'Multiple choice',
...
const ACCENT = '#D97010'
const ACCENT_BG = '#FEF0DC'
const ACCENT_TEXT = '#7A4A00'

export default function AiEngageBuilderModal({
  open,
  onClose,
  onSubmit,
  loading = false,
  ...
  const [config, setConfig] = useState<EngageAiConfig>(() =>
    defaultEngageAiConfig(subject, gradeLevel)
  )
  const [error, setError] = useState('')

  useEffect(() => {
    if (!open) return
    setConfig(defaultEngageAiConfig(subject, gradeLevel))
    setError('')
  }, [open, subject, gradeLevel])

  if (!open) return null

  const preview = buildEngageAiPreview(config)
  const total = engageMixTotal(config.typeMix)

  function updateConfig(patch: Partial<EngageAiConfig>) {
    setConfig(prev => ({ ...prev, ...patch }))
    setError('')
  }

  function setPreset(preset: EngageMixPreset) {
    setConfig(prev => ({
      ...prev,
      mixPreset: preset,
      typeMix: preset === 'custom' ? prev.typeMix : engageMixForPreset(preset, prev.totalCount),
    }))
    setError('')
  }

  function setTotalCount(count: number) {
    const totalCount = Math.max(MIN_ENGAGE_QUESTIONS, Math.min(MAX_ENGAGE_QUESTIONS, count))
    setConfig(prev => ({
      ...prev,
      totalCount,
      typeMix: prev.mixPreset === 'custom' ? prev.typeMix : engageMixForPreset(prev.mixPreset, totalCount),
    }))
    setError('')
  }

  function setMixField(key: EngageQuestionTypeKey, value: number) {
    const safe = Math.max(0, Math.min(MAX_ENGAGE_QUESTIONS, value))
    setConfig(prev => ({
      ...prev,
      mixPreset: 'custom',
  ...
  async function handleSubmit() {
    const validation = validateEngageAiConfig(config)
    if (!validation.ok) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Could not draft questions. Try again.')
    }
  }
      ...
      >
        <div style={{ padding: '20px 22px 14px', borderBottom: '0.5px solid var(--bg2)' }}>
          <p style={{ fontSize: 16, fontWeight: 600, color: 'var(--near-black)', marginBottom: 4 }}>
            Draft live quiz with AI
          </p>
          <p style={{ fontSize: 13, color: 'var(--mid-grey)', lineHeight: 1.5 }}>
            Set the topic, question count, and mix. Review the preview before drafting.
          </p>
        </div>
        ...
        <div style={{ padding: '18px 22px' }}>
          ...
          {loading && (
            <div style={{
              background: ACCENT_BG, borderRadius: 8, padding: '12px 14px', marginBottom: 14,
            }}>
              <p style={{ fontSize: 13, color: ACCENT_TEXT, fontWeight: 500 }}>
                {loadingMessage || 'Drafting your live quiz questions...'}
              </p>
            </div>
          )}
          ...
          <input
            ...
            placeholder="e.g. Photosynthesis for JHS Science"
            disabled={loading}
          ...
          <input
            type="range"
            min={MIN_ENGAGE_QUESTIONS}
            max={MAX_ENGAGE_QUESTIONS}
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    background: active ? ACCENT_BG : 'var(--white)',
                    cursor: loading ? 'not-allowed' : 'pointer',
                    fontFamily: 'inherit',
                  }}
                >
                  <p style={{ fontSize: 13, fontWeight: 600, color: active ? ACCENT : 'var(--near-black)' }}>
                    {preset.label}
                  </p>
          ...
          {config.mixPreset === 'custom' && (
            <div style={{
              ...
              {(Object.keys(TYPE_LABELS) as EngageQuestionTypeKey[]).map(key => (
                <div key={key}>
              ...
              <p style={{ gridColumn: '1 / -1', fontSize: 12, color: total > MAX_ENGAGE_QUESTIONS ? '#C23B2A' : 'var(--mid-grey)' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
            {ENGAGE_PACE_OPTIONS.map(opt => {
              const active = config.pace === opt.id
              return (
                <button
                  key={opt.id}
                  type="button"
                  onClick={() => updateConfig({ pace: opt.id })}
                  disabled={loading}
                  style={{
                    flex: 1,
                    height: 40,
                    ...
                    border: active ? `1.5px solid ${ACCENT}` : '1px solid var(--border)',
                    background: active ? ACCENT_BG : 'var(--white)',
                    color: active ? ACCENT_TEXT : 'var(--mid-grey)',
                    fontSize: 12,
                    fontWeight: 600,
                    cursor: loading ? 'not-allowed' : 'pointer',
                    fontFamily: 'inherit',
        ...
        <div style={{
          ...
          <button
            ...
            disabled={loading}
            style={{
              background: loading ? 'var(--bg2)' : ACCENT,
